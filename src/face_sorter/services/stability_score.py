...
try:
    ...
    logger.error(f"Required libraries missing: {e}. Install with: pip install onnxruntime transformers")
...
async def load_stability_model(
    ...
    if not path_obj.exists():
        if was_model_path_provided:
            logger.warning(f"Specified model path {model_path} does not exist. Attempting auto-download...")
        ...
        try:
            ...
            ort_model = await asyncio.to_thread(
                ORTModelForImageClassification.from_pretrained, 
                model_name, 
                export=True
            )
            ...
            logger.error("optimum is required for auto-download. Install with: pip install optimum[exporters]")
    ...
    if 'CoreMLExecutionProvider' in providers and '_int8' in model_path:
        logger.warning("Disabling CoreMLExecutionProvider because it produces NaNs with int8 models.")
    ...
    try:
        ...
        try:
            processor = await asyncio.to_thread(AutoImageProcessor.from_pretrained, model_name, local_files_only=True)
        except Exception:
